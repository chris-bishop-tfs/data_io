...
class BaseConnection(abc.ABC):
  ...
  def _check_source(self, data_frame):
    ...
    try:
      ...
      logging.debug(f'{self.url}: Exists: {exists}, Has Data: {has_data}')
...
#   filter_str: str=field(init=True)
...
#     # Run existence and has_data checks
#     _, has_data = self._check_source(*largs, **kwargs)

#     return has_data

#   def exists(self, *largs, **kwargs):
    
#     exists, _ = self._check_source(*largs, **kwargs)

#     return exists


# Gitx tf_transactions_mapped
gitx_url = 'redshift://user@gitx-ops-data-warehouse.ccihmrvnkut2.us-east-1.redshift.amazonaws.com:5439/gitx.dev_ds.division_event_summary'
s3_url = 's3a://tfsds-lsg-test/data/cdw/cdwcm/f_email_resp'
url = gitx_url
# url = s3_url
connection = build_connection(url)

# ds = DataSource(connection, 'select 1 from dev_ds.division_event_summary limit 1')

data = connection.read(query='select * from dev_ds.division_event_summary limit 1')
...
# connection.read().filter(sf.expr("SRC_SYS_CD='SP'")).take(1)
# connection._check_filter(filter_str="SRC_SYS_CD='SP'")
connection.check_filter("DIVISION='GSD'")

# COMMAND ----------

@define
class DataSourcePart(DataSource):
  """
  XXX
  """
...
class DataSourceBuilder(BaseBuilder):
  """
  Build out an individual data source
  """
...
class DicttoFilter():
  """
  Convert a dictionary to a filter
  """
  pass
...
def build_filter(filter_specs, *largs, **kwargs):
...
# Gitx tf_transactions_mapped
gitx_url = 'redshift://user@gitx-ops-data-warehouse.ccihmrvnkut2.us-east-1.redshift.amazonaws.com:5439/gitx.dev_ds.division_event_summary'
gma_url = 'oracle://user@rds-gma-p.csg1cie9bwnx.us-east-1.rds.amazonaws.com:1521/GMOA01P.analytics_user.f_emsg_email'
s3_url = 's3a://tfsds-lsg-test/data/cdw/cdwcm/f_email_resp'
oracle_url = 'oracle://user@cdwprd-rac-db.thermo.com:1521/cdwprd_users.cdwcm.f_email_resp'
# connection = build_connection(gitx_url)
# connection = build_connection(s3_url)
connection = build_connection(oracle_url)

# _check_source(connection, query=f'SELECT 1 FROM {connection.location.schema}.{connection.location.table} WHERE ROWNUM=1')
connection._check_source(query=f'SELECT 1 FROM {connection.location.schema}.{connection.location.table} WHERE ROWNUM=1')
...
connection.read(query=f'SELECT 1 FROM {connection.location.schema}.{connection.location.table} WHERE ROWNUM <= 1')
